...
def ks_read(self, records):
    if self.name == 'ks_list_view_fields' or self.name == 'ks_list_view_group_fields':
        ...
        query = """ SELECT {rel}.{id1}, {rel}.{id2} FROM {rel}, {from_c}
                    WHERE {where_c} AND {rel}.{id1} IN %s AND {rel}.{id2} = {tbl}.id {limit}
                """.format(
                rel=self.relation,
                id1=self.column1,
                id2=self.column2,
...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_multiplier_active = fields.Boolean(
        string="Apply Multiplier", default=False,
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=[]):
        ...
        if rec.ks_date_filter_field:
            if not rec.ks_date_filter_selection or rec.ks_date_filter_selection == "l_none":
                ...
                if self._context.get('ksDateFilterSelection', False) and self._context['ksDateFilterSelection'] not in ['l_none', 'l_custom']:
                    ks_date_data = ks_get_date(self._context.get('ksDateFilterSelection'), self._context.get('tz', self.env.user.tz),
                ...
                if rec.ks_date_filter_selection and rec.ks_date_filter_selection != 'l_custom':
                    ks_date_data = ks_get_date(rec.ks_date_filter_selection, self._context.get('tz', self.env.user.tz), rec.ks_date_filter_field.ttype)
                    selected_start_date = ks_date_data["selected_start_date"]
                    selected_end_date = ks_date_data["selected_end_date"]
                else:
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and \
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                if (rec.ks_chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                    ...
                    ks_available_types = [
                        'ks_bar_chart', 'ks_horizontalBar_chart', 'ks_line_chart', 'ks_area_chart'
                    ]
                    if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in ks_available_types and rec.ks_chart_groupby_type == "date_type":
                        ...
                        if rec.ks_date_filter_selection == "l_none":
                            ...
                            if rec.ks_date_filter_selection == "l_custom":
                                ...
                                ks_date_data = ks_get_date(rec.ks_date_filter_selection, self._context.get('tz', self.env.user.tz),
                                                           rec.ks_date_filter_field.ttype)
                                selected_start_date = ks_date_data["selected_start_date"]
                                selected_end_date = ks_date_data["selected_end_date"]
                        if selected_start_date and selected_end_date:
                ...
                if len(ks_chart_measure_field) != 0 or rec.ks_chart_data_count_type == 'count':
                    ...
                    if rec.ks_chart_sub_groupby_type == 'date_type' and rec.ks_chart_date_sub_groupby:
                        ks_chart_sub_groupby_field = rec.ks_chart_relation_sub_groupby.name + ":" + ks_chart_date_sub_groupby
                    else:
                    ...
                    try:
                        ks_chart_record = self.env[rec.ks_model_name].read_group(
                            ks_chart_domain,
                            list(set(
                                ks_chart_measure_field_with_type + ks_chart_measure_field_with_type_2 + [
                        ...
                        if res.get(ks_chart_groupby_relation_fields[0], False):
                            if rec.ks_chart_groupby_type == 'date_type':
                                ...
                                label = dict(self.env[rec.ks_model_name].fields_get(
                                    allfields=[ks_chart_groupby_relation_fields[0]])[ks_chart_groupby_relation_fields[0]]['selection'])[selection]
                            ...
                            if rec.ks_chart_data_count_type != 'count':
                                for ress in rec.ks_chart_measure_field:
                                    if rec.ks_chart_sub_groupby_type == 'date_type':
                                        if res[ks_chart_groupby_relation_fields[1]] is not False:
                                            labels.append(res[ks_chart_groupby_relation_fields[1]].split(" ")[0] + " " + ress.field_description)
                                        else:
                                        ...
                                        if res[ks_chart_groupby_relation_fields[1]] is not False:
                                            ...
                                            labels.append(dict(self.env[rec.ks_model_name].fields_get(
                                                allfields=[ks_chart_groupby_relation_fields[1]])[ks_chart_groupby_relation_fields[1]]['selection'])[
                                    ...
                                    for ress in rec.ks_chart_measure_field_2:
                                        if rec.ks_chart_sub_groupby_type == 'date_type':
                                            if res[ks_chart_groupby_relation_fields[1]] is not False:
                                                ...
                                                labels_2.append(str(res[ks_chart_groupby_relation_fields[1]]) + " " + ress.field_description)
                                        elif rec.ks_chart_sub_groupby_type == 'selection':
                                            selection = res[ks_chart_groupby_relation_fields[1]]
                                            labels_2.append(dict(self.env[rec.ks_model_name].fields_get(
                                                allfields=[ks_chart_groupby_relation_fields[1]])[ks_chart_groupby_relation_fields[1]][
                                                    'selection'])[selection] + " " + ress.field_description)
                                        elif rec.ks_chart_sub_groupby_type == 'relational_type':
                                            if res[ks_chart_groupby_relation_fields[1]] is not False:
                                                labels_2.append(
                                            ...
                                            labels_2.append(str(
                                                res[ks_chart_groupby_relation_fields[1]]) + " " + ress.field_description)
                                ...
                                if rec.ks_chart_sub_groupby_type == 'date_type':
                                    ...
                                    selection = res[ks_chart_groupby_relation_fields[1]]
                                    labels.append(dict(self.env[rec.ks_model_name].fields_get(
                                        allfields=[ks_chart_groupby_relation_fields[1]])[ks_chart_groupby_relation_fields[1]]['selection'])[selection])
                                elif rec.ks_chart_sub_groupby_type == 'relational_type':
                                    if res[ks_chart_groupby_relation_fields[1]] is not False:
                                        labels.append(res[ks_chart_groupby_relation_fields[1]][1]._value)
                                    else:
                    ...
                    if len(ks_data) != 0:
                        ...
                        if rec.ks_goal_enable and rec.ks_standard_goal_value and rec.ks_dashboard_item_type in ks_available_types:
                            goal_dataset = []
                            length = len(ks_chart_data['datasets'][0]['data'])
                            for i in range(length):
                                goal_dataset.append(rec.ks_standard_goal_value)
            ...
            if self.ks_chart_groupby_type == 'relational_type':
                ...
                try:
                    ks_list_view_records = self.env[self.ks_model_name].read_group(
                        ks_chart_domain, ks_list_fields,
    ...
    def get_target_list_view_data(self, ks_list_view_records, rec, ks_list_fields, ks_group_by,
        ...
        for res in ks_list_view_records:
            ...
            for field in ks_list_fields[1:]:
                ks_list_records[res[ks_group_by]]['measure_field'].append(res[field])
    ...
    def ks_get_previous_period_data(self, rec):
        ...
        if ks_previous_period:
            ks_date_data = ks_get_date(ks_previous_period, self._context.get('tz', self.env.user.tz), rec.ks_date_filter_field.ttype)
    ...
    def ks_set_date_filter_2(self):
        for rec in self:
            if (not rec.ks_date_filter_selection_2) or rec.ks_date_filter_selection_2 == "l_none":
                ...
                ks_date_data = ks_get_date(rec.ks_date_filter_selection_2, self._context.get('tz', self.env.user.tz), rec.ks_date_filter_field_2.ttype)
                ...
                if selected_start_date and selected_end_date:
                    if rec.ks_compare_period_2:
                        ...
                        if rec.ks_compare_period_2 > 0:
                            selected_end_date = selected_end_date + (selected_end_date - selected_start_date) * ks_compare_period_2
                            if rec.ks_date_filter_field.ttype == "date" and rec.ks_date_filter_selection == 'l_day':
                                selected_end_date = selected_end_date + timedelta(days=ks_compare_period_2)
                        elif rec.ks_compare_period_2 < 0:
                            selected_start_date = selected_start_date - (selected_end_date - selected_start_date) * ks_compare_period_2
                            if rec.ks_date_filter_field.ttype == "date" and rec.ks_date_filter_selection == 'l_day':
                ...
                if ks_chart_measure_field:
                    if ks_chart_measure_field_2:
                        ...
                        for field_rec in ks_chart_measure_field_2:
                            ks_groupby_equal_measures = res.get(ks_chart_groupby_relation_field + "_count") or res.get("__count") if res.get(ks_chart_groupby_relation_field + "_count") or res.get("__count") and ks_chart_measure_field_2_ids[index] == ks_chart_groupby_relation_field_id else 1
                            try:
                    ...
                    for field_rec in ks_chart_measure_field:
                        ks_groupby_equal_measures = res.get(ks_chart_groupby_relation_field + "_count") or res.get("__count") if res.get(ks_chart_groupby_relation_field + "_count") or res.get("__count") and ks_chart_measure_field_ids[index] == ks_chart_groupby_relation_field_id else 1
                        try:
                            if res.get('__count', False):
                                data = res[field_rec] * ks_groupby_equal_measures \
                                    if chart_count == 'sum' else \
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ...
            if action_line.ks_item_action_field.ttype == 'many2one':
                ...
                for res in ks_list_view_records:
                    ...
                    counter = 0
                    data_row = {
                        'id': res[action_line.ks_item_action_field.name][0] if res[action_line.ks_item_action_field.name] else res[action_line.ks_item_action_field.name],
                        'data': [],
    ...
    def ks_get_start_end_date(self, model_name, ks_chart_groupby_relation_field, ttype, ks_chart_domain,
        ...
        if model_field_start_date and model_field_end_date:
            goal_model_start_date = \
                self.env['ks_dashboard_ninja.item_goal'].search([
                    ('ks_goal_date', '>=', model_field_start_date.strftime("%Y-%m-%d")),
                    ('ks_goal_date', '<=', model_field_end_date.strftime("%Y-%m-%d"))
                ], limit=1, order='ks_goal_date ASC')['ks_goal_date']
            goal_model_end_date = \
                self.env['ks_dashboard_ninja.item_goal'].search([
                    ('ks_goal_date', '>=', model_field_start_date.strftime("%Y-%m-%d")),
                    ('ks_goal_date', '<=', model_field_end_date.strftime("%Y-%m-%d"))],
                    limit=1, order='ks_goal_date DESC')['ks_goal_date']
        else:
