odoo.define('ks_dashboard_ninja.ks_dashboard', function(require) {
    ...
    const KsDashboardNinja = AbstractAction.extend({
        ...
        on_attach_callback: function() {
            $.when(this.ks_fetch_items_data()).then((result) => {
        ...
        _ksOnDnLayoutMenuSelect: function(ev){
            const selected_layout_id = $(ev.currentTarget).data('ks_layout_id');
        ...
        ksOnLayoutSelection: function(layout_id){
            ...
            _(this.gridstackConfig).each((x,y)=>{
                this.grid.update(this.$el.find(".grid-stack-item[gs-id=" + y + "]")[0],{ x:x['x'], y:x['y'], w:x['w'], h:x['h'],autoPosition:false});
            });
