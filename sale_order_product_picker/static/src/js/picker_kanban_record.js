  ...
  KanbanRecord.include({
    ...
    _getLinesOfRecord: async function(list) {
      ...
      for (var key in lines) {
        ...
        if (
          ...
          (!list_changes || !list_changes.filter(ob => ob.id === key).length)
        ) {
          line.deleted = true;
        }
    ...
    _openMultiLineModalPicker: async function(list, lines) {
      ...
      if ($new_list.find(".o_list_view").length !== 0) {
        // Check if tree mode
        $new_list.find("tr:has(.o_field_x2many_list_row_add)").addClass("d-none");
      ...
      for (var line of lines) {
        var line_showed = list.value.data.filter(el => el.id === line.line.id)[0];
